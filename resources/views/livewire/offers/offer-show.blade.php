    ...
    <x-card title="{{ $offer->code ?? 'N/A' }}">
            ...
            <div class="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                    ...
                    <div class="flex items-center gap-2">
                        <select wire:change="updateOfferStatus($event.target.value)"
                            ...
                            @foreach ($statuses as $statusOption)
                                <option value="{{ $statusOption }}" {{ $offer->status === $statusOption ? 'selected' : '' }}>
                ...
                <div class="flex justify-between gap-2 w-full">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2">
                        @can('update-offer', $offer)
                            <button wire:click="editOffer" type="button" class="btn btn-warning btn-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
            <div>
                ...
                <span
                    class="badge badge-{{ $offer->status === 'draft' ? 'secondary' : ($offer->status === 'sent' ? 'primary' : ($offer->status === 'accepted' ? 'success' : 'danger')) }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 pt-2 border-t">
            ...
            <div>
                ...
                <p class="form-control-plaintext mb-0">
                    {{ $offer->created_at ? $offer->created_at->format('Y-m-d H:i') : 'N/A' }}</p>
            </div>

            <div>
                <x-input-label :value="__('Updated At')" />
                <p class="form-control-plaintext mb-0">
                    {{ $offer->updated_at ? $offer->updated_at->format('Y-m-d H:i') : 'N/A' }}</p>
            </div>
        </div>
    </x-card>
                        ...
                        <div class="mb-3">
                            ...
                            <p class="form-control-plaintext">
                                @if ($item->product)
                                    ...
                                    {{ $item->product->spec->name ?? 'N/A' }}) - {{ $item->product->name }}
                                @else
                                    N/A
                                @endif
                            </p>
                                    ...
                                    <div class="border border-gray-200 rounded-lg p-2 mb-2 bg-white">
                                        <div class="grid grid-cols-3 gap-2 text-sm">
                                            ...
                                            <div>
                                                ...
                                                <p class="mb-1">{{ number_format($ingredient->percentage ?? 0, 2) }}%
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="border border-green-200 rounded-lg p-2 sm:p-4 h-full bg-green-50">
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                ...
                                <p class="form-control-plaintext">{{ number_format($item->kg_per_package ?? 0, 3) }}
                                </p>
                            </div>

                            <div>
                                <x-input-label :value="__('Cost/Package')" />
                                <p class="form-control-plaintext">{{ number_format($item->one_package_cost ?? 0, 2) }}
                                </p>
                            </div>
                        </div>
                        ...
                        @can('view-product-costs')
                            <div class="mb-3">
                                ...
                                <p class="form-control-plaintext font-bold">
                                    {{ number_format($item->total_packing_cost ?? 0, 2) }}</p>
                            </div>
                        @endcan
                        ...
                        @can('view-product-costs')
                            <div class="mb-3">
                                ...
                                <p class="form-control-plaintext">{{ number_format($item->base_cost_currency ?? 0, 2) }}
                        ...
                        <div>
                            ...
                            <p class="form-control-plaintext font-bold text-green-600">
                                {{ number_format($item->fob_price ?? 0, 2) }} {{ $offer->currency->code ?? 'N/A' }}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="border border-yellow-200 rounded-lg p-2 sm:p-4 h-full bg-yellow-50">
                            ...
                            <div class="grid grid-cols-3 gap-1 text-sm">
                                ...
                                @can('view-product-costs')
                                    <div>
                                        ...
                                        <p class="font-bold">{{ number_format($item->freight_total_cost ?? 0, 2) }}
                                            {{ $offer->currency->code ?? 'N/A' }}</p>
                                    </div>
                                @endcan
                            </div>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="grid grid-cols-3 gap-1 text-sm">
                                ...
                                @can('view-product-costs')
                                    <div>
                                        <small class="text-muted">Total:</small>
                                        <p class="font-bold">{{ number_format($item->sterilization_total_cost ?? 0, 2) }}
                                            {{ $offer->currency->code ?? 'N/A' }}</p>
                                    </div>
                                @endcan
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="grid grid-cols-3 gap-1 text-sm">
                                ...
                                <div>
                                    <small class="text-muted">Type:</small>
                                    <p>{{ ucfirst(str_replace('_', ' ', $item->agent_commission_type ?? 'fixed')) }}
                                    </p>
                                </div>
                                @can('view-product-costs')
                                    <div>
                                        <small class="text-muted">Total:</small>
                                        <p class="font-bold">
                                            {{ number_format($item->agent_commission_total_cost ?? 0, 2) }}
                                            {{ $offer->currency->code ?? 'N/A' }}</p>
                                    </div>
                                @endcan
                            </div>
        ...
        <x-card title="Offer Summary">
            ...
            <div class="grid grid-cols-3 sm:grid-cols-3 gap-4 mt-4 pt-4 border-t">
                <div class="text-center">
                    ...
                    <h4 class="text-green-600 text-xl font-bold">{{ number_format($offer->total_profit ?? 0, 2) }}
                    </h4>
                </div>
                <div class="text-center">
                    <h5 class="text-gray-600 text-sm">Profit Percentage</h5>
                    <h4 class="text-green-600 text-xl font-bold">
                        {{ number_format($offer->profit_percentage ?? 0, 2) }}%</h4>
                </div>
                <div class="text-center">
